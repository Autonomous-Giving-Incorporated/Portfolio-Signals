...
export async function mountBrandConfiguration(root) {
  ...
  const setForm = (config) => { current = safeConfig(config); for (const [key, value] of Object.entries(current)) if (key !== 'theme' && key !== 'assets') form.elements[key].value = value; form.elements.primary.value = current.theme.primary; form.elements.accent.value = current.theme.accent; form.elements.background.value = current.theme.background; form.elements.logo_path.value = current.assets.logo_path || ''; form.elements.icon_path.value = current.assets.icon_path || ''; form.elements.hero_path.value = current.assets.hero_path || ''; renderPreview(); };
