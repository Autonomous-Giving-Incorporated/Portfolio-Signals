...
const FALLBACK_CONFIG = {
  ...
  approvals: { decision_approvers: 1 },
...
function safeConfig(config = {}) {
  return {
    ...
    approvals: { decision_approvers: Number(config.approvals?.decision_approvers) === 2 ? 2 : 1 },
    theme: {
...
export async function mountBrandConfiguration(root) {
  ...
  const setForm = (config) => { current = safeConfig(config); for (const key of ['organization_name', 'product_name', 'campaign_title', 'campaign_tagline']) form.elements[key].value = current[key]; form.elements.module_sponsors.checked = current.modules.sponsors; form.elements.module_grants.checked = current.modules.grants; form.elements.decision_approvers.value = String(current.approvals.decision_approvers); form.elements.primary.value = current.theme.primary; form.elements.accent.value = current.theme.accent; form.elements.background.value = current.theme.background; form.elements.logo_path.value = current.assets.logo_path || ''; form.elements.icon_path.value = current.assets.icon_path || ''; form.elements.hero_path.value = current.assets.hero_path || ''; renderPreview(); };
