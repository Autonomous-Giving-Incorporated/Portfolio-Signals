...
| OBSERVED | 2026-08-23 — `auth-email-webhook` v2 ACTIVE (`verify_jwt=false`). `RESEND_WEBHOOK_SECRET` set on `utdioxwiskzatwoejgiu`. Unsigned POST → `401 invalid_signature`. Signed Svix POST → `200`. All 10 `auth_email_dispatches` now have `delivery_status` from Resend-confirmed events: 6 `platform_admin_magic_link` delivered, 3 `tenant_member_magic_link` delivered, 1 `tenant_member_magic_link` bounced. One `auth_email_alerts` bounce row. `ALERT_WEBHOOK_URL` still unset. |
...
OBSERVED 2026-08-23: Resend webhook endpoint already enabled at that URL; Edge `RESEND_WEBHOOK_SECRET` set; signed feedback loop writes `delivery_status` for all current dispatches. Do not create a second webhook.
