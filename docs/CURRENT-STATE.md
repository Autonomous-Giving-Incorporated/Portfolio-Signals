...
| Role-aware Resend magic links (`auth-email`) | **Function deploy OBSERVED** 2026-08-22 on `utdioxwiskzatwoejgiu` (`auth-email` v3 + `auth-email-webhook` v1; IP-budget / delivery / alerts migrations applied). Live handler rejects localhost and unlisted origins. `RESEND_WEBHOOK_SECRET` **OBSERVED unset** (webhook returns `function_not_configured`). `RESEND_API_KEY` / `AUTH_EMAIL_FROM` **NOT_COMPUTABLE** (no send executed). P8 synthetic drill PENDING. [AUTH-ROLES-AND-EMAILS.md](AUTH-ROLES-AND-EMAILS.md) |
