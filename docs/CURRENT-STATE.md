...
| Givebutter / Donorbox P1 webhooks (SPEC-026 v1.1.0) | CODE_SHIPPED in-repo (`POST /webhooks/givebutter`, `POST /webhooks/donorbox`); merged [PR #52](https://github.com/Autonomous-Giving-Incorporated/Portfolio-Signals/pull/52) `4c38323`; atomic `am_credit_pot` merged [PR #59](https://github.com/Autonomous-Giving-Incorporated/Portfolio-Signals/pull/59) `36091e2` and OBSERVED applied on `utdioxwiskzatwoejgiu` (`20260823192249_am_credit_pot`); not a live gift; not READY; operator-owned pointing — [GIFT-CONNECTORS.md](GIFT-CONNECTORS.md) |
...
| Role-aware Resend magic links (`auth-email`) | **Function deploy + P8 tenant-member send + isolation-director send/click OBSERVED** 2026-08-22 on `utdioxwiskzatwoejgiu` / Mailosaur `qpbqeifu`. Click reached `autogive.app/portfolio-signals/workspace`. **Webhook secret OBSERVED set** 2026-08-23 (unsigned `401`; signed `200`). All 10 dispatches have Resend-backed `delivery_status` (6 platform-admin delivered, 3 tenant-member delivered, 1 tenant-member bounced + alert row). MFA drill still PENDING. [AUTH-ROLES-AND-EMAILS.md](AUTH-ROLES-AND-EMAILS.md) |
